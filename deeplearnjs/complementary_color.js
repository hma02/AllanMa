...
function createFullyConnectedLayerSigmoid(
    graph, inputLayer, layerIndex,
    sizeOfThisLayer, includeSigmoid = true, includeBias = true) {
    return graph.layers.dense(
        'fully_connected_' + layerIndex, inputLayer, sizeOfThisLayer,
        includeSigmoid ? (x) => graph.sigmoid(x) : undefined, includeBias);
}
...
function train1Batch(shouldFetchCost) {
    ...
    const learningRate =
        initialLearningRate * Math.pow(0.85, Math.floor(step / 42));
...
function trainAndMaybeRender() {
    ...
    for (let i = 0; i < localStepsToRun; i++) {
        cost = train1Batch(i === localStepsToRun - 1);
        step++;
    }
